...
void save(char *filename)
{
    ...
    {
        if (!strcmp(savefile, "")) /* and no previous loaded file */
        {
            (debug) ? printf("No previously loaded file, setting to default name\n") : NULL;
            strcpy(file, "filesystem.txt"); /* set to default name */
        }
        else
        {
            (debug) ? printf("Previously loaded file present\n") : NULL;
            strcpy(file, savefile); /* set to previosuly loaded file */
        }
    }
    else
    {
        (debug) ? printf("Save file name provided\n") : NULL;
...
void mkdir(char *pathname)
{
    ...
    if (location == NULL) /* if no pathname is provided */
    {
        return;
    }

    NODE *dupe_search = location->childPtr; /* check if dir name is already taken */
    while (dupe_search)
    {
        ...
        dupe_search = dupe_search->siblingPtr; /* eventually goes to NULL */
    }
    insert_node(location, bname, DIRECTORY_TYPE); /* no duplicate names found, insert */
}

NODE *insert_node(NODE *pcur, char *name, char type)
{
    /* inserting into pcur Dir */
